...
def load_significant(
    ...
    """Significant sites for one test, restricted to the MAGs of interest.
    ...
    for chunk in pd.read_csv(path, sep="\t", usecols=usecols, chunksize=CHUNK):
        sel = (
            (chunk["test_type"] == test_type)
            & (chunk["q_value"] < Q_THRESHOLD)
            & chunk["mag_id"].isin(mags)
        )
        if group is not None:
            sel &= chunk["group_analyzed"] == group
        if sel.any():
            kept.append(chunk.loc[sel, ["mag_id", "contig", "position"]])

    out = (
        pd.concat(kept, ignore_index=True)
        if kept
        else pd.DataFrame(columns=["mag_id", "contig", "position"])
    )
    ...
    if dupes:
        ...
        raise SystemExit(1)
    logger.info(
        f"{path.name} [{label}]: {len(out):,} significant sites "
        f"across {out.mag_id.nunique()} of the requested MAGs"
    )
    return out
...
def main() -> int:
    ...
    out[["n_sites_both", "n_contigs_both"]] = (
        out[["n_sites_both", "n_contigs_both"]].fillna(0).astype(int)
    )
